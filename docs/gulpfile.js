      ...
      .pipe(through2.obj(function(demo, enc, next) {
        ...
        var demoIndex = _(demos)
          ...
          .map(function(moduleDemos, moduleName) {
            return {
              name: moduleName,
              label: humanizeCamelCase( moduleName.split('.').pop() ),
              demos: moduleDemos,
              url: '/demo/' + moduleName
            };
          })
  ...
  function generateDemos() {
    return gulp.src('src/**/demo*/')
      .pipe(through2.obj(function(file, enc, next) {
        ...
        // Remove everything except the path starting after 'src/'
        // gives us back 'components/toast/demo1/' for example
        var srcPath = file.path.replace(
          path.resolve(file.path, '..', '..', '..') + '/', ''
        );
        var split = srcPath.split('/');
        var moduleName = 'material.' + split[0] + '.' + split[1];
        var demoId = split[1] + split[2];

        gulp.src(file.path + '**/*', { base: file.path })
          .pipe(gulpif(/.css$/, transformCss()))
          .pipe(gulp.dest('dist/docs/demo-partials/' + demoId))
          .pipe(utils.generateDemoObject(moduleName, demoId))
